...
export class UserScheduleService extends CrudService<UserSchedule> {
  ...
  public async approveUserScheduleRequest(
    request_ids: string[],
    ...
    try {
      ...
      await queryRunner.manager.save(
        UserScheduleRequest,
        find_requests.map((request) => ({
          ...request,
          status: UserScheduleRequestStatus.APPROVED,
        })),
      );

      const approved_schedule = queryRunner.manager.save(
        UserSchedule,
        find_requests.map(({ work_from, work_to, user_id }) => ({
          user_id,
          work_from,
          work_to,
        })),
  ...
  public async declineUserScheduleRequest(
    request_ids: string[],
    ...
    return await this.userScheduleRequestService.createOrUpdateUserScheduleRequest(
      find_requests.map((request) => ({
        ...request,
